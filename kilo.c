...
void editorSave(void) {
  // user can't create new file yet... handle this later
  if (E.filename == NULL) {
  ...
  // added error handling to editorSave();
  if (fd != -1) {
  ...
  switch (c) {
    ...
    case CTRL_KEY('q'):
      if (E.dirty && quit_times > 0) {
        editorSetStatusMessage("WARNING!!! FIle has unsaved changes. " "Press C-q %d more times to quit", quit_times);
        quit_times--;
        return;
      }
